...
if (!function_exists('get_join')) {
    function get_join($tableFrom, $tableTo, $joinCond, $where = [], $select = null, $groupBy = null, $order_col = null, $order_by = 'desc', $limit = null, $limit_offset = null, $where_in = null, $like = null, $not_like = null)
    {
        //get main CodeIgniter object
        $ci =& get_instance();
...
if(!function_exists('read_sum')){
    function read_sum($table, $column, $where=array()){
        $ci = & get_instance();
        $ci->db->select_sum($column);
        $ci->db->where($where);
...
if(!function_exists('readGroupBy')){
    function readGroupBy($table, $groupBy, $where=array(), $orderBy="id", $sort="desc"){
        $ci = & get_instance();
        $ci->db->select('*');
        $ci->db->group_by($groupBy);
...
// get student balance
if (!function_exists('get_student_due')) {
    function get_student_due($student_id = null, $class = null, $section = null, $type = null)
    {
        //get main CodeIgniter object
        $ci =& get_instance();
        ...
        //get data from databasea
        if (!empty($student_id) && !empty($class) && !empty($section) && !empty($type)) {

            $totalAmount      = $ci->db->query("SELECT IFNULL(SUM(sp.amount), 0) AS amount FROM payment_setting as mps JOIN set_payment as sp ON mps.set_payment_id=sp.id WHERE sp.class='$class' AND sp.section='$section' AND sp.type='$type'")->row();
            $paidAmount       = $ci->db->query("SELECT IFNULL(SUM(amount), 0) AS amount FROM `payment` WHERE student_id='$student_id' AND class='$class' AND section='$section' AND type='$type' AND status='approved' AND trash=0")->row();
            $rocketPaidAmount = $ci->db->query("SELECT IFNULL(SUM(amount), 0) AS amount FROM `rocket_payments` WHERE student_id='$student_id' AND class='$class' AND section='$section' AND type='$type'")->row();
            
            $totalAmount = (!empty($totalAmount->amount) ? $totalAmount->amount : 0);
            $paidAmount  = (!empty($paidAmount->amount) ? $paidAmount->amount : 0);
            $rocketPaidAmount  = (!empty($rocketPaidAmount->amount) ? $rocketPaidAmount->amount : 0);
            
            $due = $totalAmount - ($paidAmount + $rocketPaidAmount);
...
if (!function_exists('get_select')) {
    function get_select($inputArray = null, $select = null)
    {
        if (!empty($inputArray) && !empty($select)) {
