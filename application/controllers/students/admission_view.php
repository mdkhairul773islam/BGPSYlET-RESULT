...
class Admission_view extends Admin_Controller {
    ...
    public function show() {
        ...
        if($this->input->post("viewQuery")) {
            ...
            $student_section = trim($this->input->post('student_section'));
            $student_group   = trim($this->input->post('student_group'));
            $students_roll   = trim($this->input->post('students_roll'));
            
            $where_search=array();
            if($session){
                $where_search['session'] = $session;
            }if($class){
                $where_search['class'] = $class;
            }if($student_section){
                $where_search['student_section'] = $student_section;
            }if($student_group){
                $where_search['student_group'] = $student_group;
            }if($students_roll){
                $where_search['students_roll'] = $students_roll;
            }
            $this->data['student_info'] = $this->action->read('students',$where_search); 
        }else{
