...
class Testimonial extends Admin_Controller {
    ...
    public function index($emit = NULL) {       
        ...
        $this->data['subMenu'] = 'data-target="testimonial"';
        ...
        $this->load->view('components/testimonial/nav', $this->data);
    ...
    public function certificate($roll=null){
        ...
        $this->data['serial']  = generateUniqueSerial('take_testomnial', 4); 
        ...
        if ($this->input->post("viewQuery")) {
            ...
            $data = array(
                'date'       =>date('Y-m-d'),
                'student_id' =>$this->data['student'][0]->id, 
            );
        
         $this->action->add('take_testomnial',$data);
        }
    ...
    public function all() {       
        ...
        if(!empty($this->input->post('roll'))){
            $where['passed_student.roll'] = $this->input->post('roll');
        }
        
        $this->data['students']=get_join('take_testomnial', 'passed_student', 'take_testomnial.student_id=passed_student.id', $where, '', 'take_testomnial.student_id', 'take_testomnial.id', 'DESC');

        $this->load->view($this->data['privilege'].'/includes/header', $this->data);
        $this->load->view($this->data['privilege'].'/includes/aside', $this->data);
        $this->load->view($this->data['privilege'].'/includes/headermenu', $this->data);
        $this->load->view('components/testimonial/nav', $this->data);
        $this->load->view('components/testimonial/all', $this->data);
        $this->load->view($this->data['privilege'].'/includes/footer', $this->data);
    }
    
    
     public function certificate_show($roll=null){
         
        $this->data['meta_title'] = 'Testomonial';
        $this->data['message'] = $this->data['student'] = NULL;
        
        $this->data['serial']  = generateUniqueSerial('take_testomnial', 4); 
        $this->data['student']=$this->action->read('passed_student',['roll'=>$roll]);
